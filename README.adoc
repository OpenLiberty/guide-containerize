...
As mentioned previously, the parent image used in each `Dockerfile` contains the `full` tag that includes all of the Liberty features. This is recommended for development but when deploying to production it's recommended to use a parent image with the `kernel-slim` tag to provide a bare minimum server and only add features required by the application.
...
Replace the parent image with [hotspot=kernel-slim file=0]`icr.io/appcafe/open-liberty:kernel-slim-java11-openj9-ubi` at the top of your Dockerfile. This image contains the `kernel-slim` tag that's recommended when deploying to production.

Place [hotspot=features file=0]`RUN feature.sh` between the `COPY` commands in the `Dockerfile`. The `features.sh` script adds the Liberty features that your application is utilizing to maintain the smaller image size.

Repeat the instructions for the `system` service.
...
Now, run the following commands to stop and remove your `Docker` containers using the `full` parent image:
...
There is the `full` and `kernel-slim` images for both `inventory` and `system` services in the list of all Docker images. Note the size difference between the `full` images and the `kernel-slim` images:
----
REPOSITORY		TAG				IMAGE ID		CREATED			SIZE
...
To ensure your containers are working properly, try accessing the `system` service, which shows the system properites of the running JVM. 
...
Then, try accessing the `inventory` service, which displays the current contents of the inventory. 
