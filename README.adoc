...
In production, to minimize your image footprint, it is recommended that you use one of the `kernel-slim` images. For example, `icr.io/appcafe/open-liberty:kernel-slim-java11-openj9-ubi`.
This will install the basic server, to which you can then add all the necessary features for your application via the usage pattern detailed in the Open Liberty https://github.com/OpenLiberty/ci.docker#building-an-application-image[container image documentation^].
...
You can find all the https://github.com/OpenLiberty/ci.docker/blob/master/docs/icr-images.md[official universal base images (ubi)^] that
