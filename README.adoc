...
As mentioned when you were building the Docker images, the parent image you are using is the `full` tag which includes all the features for the `OpenLiberty` server but when deploying to production it is recommended to use a parent image with the `kernel-slim` tag to provide a bare minimum server.

To your development containers and their corresponding sizes run the following command:
[role='command']
```
...
You are replacing the parent image for your container with [hotspot=kernel-slim file=0]`icr.io/appcafe/open-liberty:kernel-slim-java11-openj9-ubi` which contains the `kernel-slim` tag indicating that this image is intended for production use.

Place the [hotspot=features file=0]`RUN feature.sh` between the two copy commands in your Dockerfile. The `features/sh` script will be able to include all the required features that is needed by your `OpenLiberty` server.

Repeat the same instructions that were done on the `inventory` service for the `system` service.
...
Run the following commands to stop and delete your Docker containers that are using the `full` image:
...
Now, run the following commands to rebuild your Docker containers with the `kernel-slim` image:
...
Once your updated containers are built, then run the following to start them:
...
To see the newly launched containers and their new sizes, run the following:
...
To ensure your containers are up and working properly visit `http://localhost:9081/inventory/systems` to check the `inventory` service, you should see `this`. Do the same for the `system` service by going to `http://localhost:9081/inventory/systems` and you should see `this but for system`.
