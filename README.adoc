...
Place [hotspot=features file=0]`RUN features.sh` between the `COPY` commands in the `Dockerfile`. The `features.sh` script adds the Liberty features that your application is required to operate.
...
http://localhost:9081/inventory/systems/[http://localhost:9081/inventory/systems/[system-ip-address]]
