...
It is recommended to add your own labels to your Docker image like the [hotspot=label file=0]`LABEL` command doing, so that those information can help you to manage your images. See https://docs.docker.com/develop/develop-images/dockerfile_best-practices/#label[Best practices for writing Dockerfiles^].
...
[role='command']
```
docker images -f "label=io.openliberty.guide.image.authors=My Name"
```
