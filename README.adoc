...
Containers enable you to easily deploy your microservices in different environments in lightweight and portable manners.
From development to production and across your DevOps environments, you can deploy your microservices consistently and
efficiently with containers. You run a container from a container image and each container image is a package of what you
...
The `system` service shows the system properties of the running JVM and can be found at the http://localhost:9080/system/properties[^] URL.
...
As each instruction is executed in a `Dockerfile`, a new Docker layer is created. These layers, known as intermediate images, are created when a change is made to your Docker image.

Every `Dockerfile` begins with a parent or base image on top of which various commands are run. For example, you can start your image from scratch and execute commands that download and install a Java runtime, or you can start from an image that already contains a Java installation.
...
image, which comes with the latest Open Liberty runtime by default. You can find all different official images at the https://hub.docker.com/_/open-liberty[open-liberty Docker Hub^] page. To pull a different open-liberty image, like `kernel`, you may define the `FROM` instruction as `FROM open-liberty:kernel`.

During the build, instead of using multiple `COPY` commands to transfer configuration files and the application itself, we are using a single `ADD` command because we have packaged the application into an archive. This eliminates having multiple layers in the process of creating the Docker image.
...
It copies local files and directories into the specified destination within your Docker image. If the source is an archive, the [hotspot=add file=0]`ADD` instruction will unpack it into the destination directory.
In this case, the `inventory` application archive [hotspot=6 file=0]`inventory.tar.gz`, that was created from running `mvn install` previously, will be extracted to the destination directory [hotspot=7 file=0]`/opt/ol`. 

The [hotspot=add file=0]`ADD` instruction needs to use the user id [hotspot=5 file=0]`1001` and group [hotspot=5 file=0]`0` because the `open-liberty` image runs by default with the `USER 1001` (non-root) user for security purposes. Without this, the files and directories copied over would be owned by the root user.
...
The `-t` flag in the `docker build` command allows the Docker image to be labelled (tagged) in the `name[:tag]` format. 
The tag for an image describes the specific image version. If the optional `[:tag]` tag is not specified, the `latest` tag will be created by default.
...
2b584282e0f5   inventory          "/opt/ol/helpers/run…"   2 seconds ago    Up 1 second      9080/tcp, 9443/tcp, 0.0.0.0:9081->9081/tcp   inventory
99a98313705f   system             "/opt/ol/helpers/run…"   3 seconds ago    Up 2 seconds     0.0.0.0:9080->9080/tcp, 9443/tcp             system
----
...
Next, you will retrieve the `system` container's IP address by using the `system` container's name defined when running the Docker containers. 
...
In this case, the IP address for the `system` service is `172.17.0.2`. Take note of this IP address in order to add the system properties to the `inventory` service. 

Point your browser to the `\http://localhost:9081/inventory/systems/pass:c[[system-ip-address]]` URL by replacing `[system-ip-address]` with the IP address you obtained earlier.
