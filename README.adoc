...
The [hotspot=copy-config hotspot=copy-war file=0]`COPY` instructions are structured as [hotspot=copy-config file=0]`COPY` [hotspot=config-userID hotspot=war-userID file=0]`[--chown=<user>:<group>]` [hotspot=inventory-config hotspot=inventory-war file=0]`<source>` [hotspot=config hotspot=config-apps file=0]`<destination>`. 
They copy local files into the specified destination within your Docker image.
In this case, the `inventory` application archive [hotspot=inventory-war file=0]`inventory.war`, created from running `mvn package` previously, is copied to the destination directory [hotspot=config-apps file=0]`/config/apps`. 
