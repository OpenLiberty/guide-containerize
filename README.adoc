...
The [hotspot=add file=0]`ADD` instruction copies local files and directories into the destination within your Docker image. The [hotspot=add file=0]`ADD` instruction also unpacks local archives into the destination directory specified in the `Dockerfile`. It is structured as [hotspot=add file=0]`ADD` [hotspot=3 file=0]`<source>` [hotspot=4 file=0]`<destination>`. 
In this case, the source is a `tar.gz` file that was created from running `mvn install` and the destination is a directory called `/opt/ol`.
...
Finally, the [hotspot=3 file=0]`inventory.tar.gz` files are copied over into [hotspot=3 file=0]`/opt/ol`.
