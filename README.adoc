...
In this case, you are using the recommended production image, 
`openliberty/open-liberty:kernel-java8-openj9-ubi`, as your parent image. If you would not like to have any additional runtime features for your `kernel` image, define the `FROM` instruction as `FROM open-liberty:kernel`. To use the default image that comes with the Open Liberty runtime, simply define the `FROM` instruction as `FROM open-liberty`. You can find all the different official images at https://hub.docker.com/_/open-liberty[open-liberty Docker Hub^].
