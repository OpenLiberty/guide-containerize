...
As mentioned previously, the parent image used in each `Dockerfile` contains the `full` tag that includes all of the Liberty features. This is recommended for development but when deploying to production it's recommended to use a parent image with the `kernel-slim` tag to provide a bare minimum server with the ability to add features that the application requires.
...
Repeat these instructions again for the `system`.
...
Verify that the images built by executing the following command to list all local `Docker` images.
...
After confirming that the images have been built, run the following commands to launch the `Docker` containers:
...
Add the system properties of your localhost to the your `inventory` service by visiting:
http://localhost:9081/inventory/systems/localhost

Then verify the addition of your localhost's system properties by accessing the `inventory` service. 
