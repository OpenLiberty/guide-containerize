...
During the build, use `COPY` commands to transfer configuration files and the application because the application ????is packaged into an archive. It eliminates multiple layers as the Docker image is created.
