...
Run the following command to see your containers and their corresponding sizes:
...
You are replacing `icr.io/appcafe/open-liberty:full-java11-openj9-ubi` with `icr.io/appcafe/open-liberty:kernel-slim-java11-openj9-ubi` which will use the `kernel-slim` tag in the Docker images. Since the server will be the bare minimum, you must include `features.sh` which will include all of the features that are required for your server.

You must do the same for the system service as well.
...
Now, run the following to rebuild your Docker containers with the `kernel-slim` image:
...
Once your updated containers are built, then run the following commands to start your containers:
...
After the containers have been started, you are able to check the size of the ne container with the following command:
