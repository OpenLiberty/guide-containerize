...
Place [hotspot=features file=0]`RUN feature.sh` between the `COPY` commands in the `Dockerfile`. The `features.sh` script adds the Liberty features that your application is required to use.

Ensure that you repeat these instructions again for the `system`.
...
Next, add the system properties of your localhost to the your `inventory` service by visiting:
http://localhost:9081/inventory/systems/localhost

Then verify the addition of your localhost's system properties to the `inventory` service by accessing. 
