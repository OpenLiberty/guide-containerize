...
During the build, instead of using multiple `COPY` commands to transfer configuration files and the application itself, we are using a single `ADD` command because we have packaged the application into an archive. This eliminates a couple of layers in the process of creating the Docker image.
