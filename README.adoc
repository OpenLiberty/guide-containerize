...
As mentioned previously, the parent image you are using in the Dockerfiles contains the `full` tag which includes all the available features for the `OpenLiberty` server. This is recommended when in development but when deploying to production it is recommended to use a parent image with the `kernel-slim` tag to provide a bare minimum server with the ability to add features as needed.
...
----
The parent image, [hotspot=kernel-slim file=0]`icr.io/appcafe/open-liberty:kernel-slim-java11-openj9-ubi`, contains the kernel-slim tag which you will use for your production image.

Place [hotspot=features file=0]`RUN feature.sh` between the `COPY` commands in the Dockerfile. The `features.sh` script will be able to include all the required features needed for your `OpenLiberty` server.

Repeat the same instructions for the `system` service.
...
Now, run the following commands to stop and remove your Docker containers using the `full` image:
...
Next, build your new Docker images with the `kernel-slim` tag:
...
There will be the `full` and `kernel-slim` iamges for both `inventory` and `system` services in the list of all Docker images. Note the size difference between the `full` images and the `kernel-slim` images:
----
REPOSITORY		TAG				IMAGE ID       CREATED       	SIZE
...
Run the following to start the `kernel-slim` images:
...
To ensure your containers are working properly, try accessing the `inventory` service, which displays the current contents of the inventory. See 
http://localhost:9081/inventory/systems

Then, try accessing the `system` service, which shows the system properites of the running JVM. See
http://localhost:9080/system/properties

Your able to add the system properties of your localhost to the `inventory` service at
