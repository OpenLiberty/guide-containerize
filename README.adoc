...
replacing **[system-ip-address]** with the IP address that was previously obtained and stored in the `SYSTEM_IP` variable. Run the following command:
```
...
// cloud-hosted guide instructions
ifdef::cloud-hosted[]
Run the Maven **package** goal to compile the test classes. Run the Maven **failsafe** goal to test the services that are running in the Docker containers by setting **-Dsystem.ip** to the IP address that was previously obtained and stored in the `SYSTEM_IP` variable.
