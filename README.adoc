...
// cloud-hosted guide instructions
ifdef::cloud-hosted[]
Run the Maven `package` goal to compile the test classes. Run the Maven `failsafe` goal to test the services that are running in the Docker containers by setting `Dsystem.ip` with the IP address that you determined previously.
