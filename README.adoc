...
It is recommended to add your labels or override the labels from `open-liberty` image by the [hotspot=label file=0]`LABEL` command, so that those information can help you to manage your images. See https://docs.docker.com/develop/develop-images/dockerfile_best-practices/#label[Best practices for writing Dockerfiles^].
...
The `Dockerfile` for the `system` service follows the same instructions as the `inventory` service, except that the [hotspot=copy-war file=1]`system.war` archive is copied over into `/config/apps`.
...
Run the Maven `failsafe` goal to test the services running in the Docker containers by replacing the `[system-ip-address]` with the IP address determined in the previous section.

[role='command']
```
mvn failsafe:integration-test -Dsystem.ip=[system-ip-address]
```
