...
The [hotspot=add file=0]`ADD` instruction is structured as [hotspot=add file=0]`ADD` [hotspot=3 file=0]`<source>` [hotspot=4 file=0]`<destination>`. 
...
In this case, the inventory archived application file [hotspot=3 file=0]`inventory.tar.gz`, that was created frmo running `mvn install`, will be extracted to the destination directory [hotspot=4 file=0]`opt/ol`. 

The [hotspot=add file=0]`ADD` instruction needs to use the user id [hotspot=2 file=0]`1001` and group [hotspot=2 file=0]`0` because the `OpenLiberty` image runs by default with the `USER 1001` (non-root). Without this, the files and directories copied over would be owned by root.
Testing tag behaviour. [hotspot=0 file=0]`0` [hotspot=1 file=0]`1` [hotspot=2 file=0]`2` [hotspot=3 file=0]`3` [hotspot=4 file=0]`4` [hotspot=5 file=0]`5` [hotspot=6 file=0]`6` [hotspot=7 file=0]`7` [hotspot=8 file=0]`8`
