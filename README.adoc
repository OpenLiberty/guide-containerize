...
The [hotspot=add file=0]`ADD` instruction copies local files and directories into the destination within your Docker image. The [hotspot=add file=0]`ADD` instruction also unpacks local archives into the destination directory specified in the `Dockerfile`. It is structured as `ADD <source> <destination>`. 
...
Finally, the `inventory.tar.gz` files are copied over into `/opt/ol`.
