...
In the [hotspot file=1]`inventory/server.xml` file, the [hotspot=httpPort
file=1]`default.http.port` variable is declared and it is used in the
[hotspot=httpEndpoint file=1]`<httpEndpoint>` element to define the service's endpoint. The default value of the
of the [hotspot=httpPort file=1]`default.http.port` variable is `9081`, however
...
When you previously ran the `inventory` container, none of the above environment variables were defined and thus the default value of `9081` was used.

Run the following commands to stop the `inventory` container and re-run with the `default.http.port` environment variable set:
...
Now, when the service is starting up, Open Liberty will find the
`default.http.port` environment variable and use it to define the value of the
[hotspot=httpPort file=0]`default.http.port` variable to be used in the HTTP
endpoint. 

The `inventory` service should now be available on the new port number you
specified. You should be able to see the contents of the inventory at
http://localhost:9091/inventory/systems[http://localhost:9091/inventory/systems^]
and you can add the system properties of your localhost at
http://localhost:9091/inventory/systems/localhost[http://localhost:9081/inventory/systems/localhost^].

You can externialize the configuration of more than just the port numbers.
