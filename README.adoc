...
`FROM` instruction as `FROM icr.io/appcafe/open-liberty:kernel-slim-java11-openj9-ubi`. To use the default image that
comes with the Open Liberty runtime, define the `FROM` instruction as `FROM icr.io/appcafe/open-liberty`. 
You can find all the https://github.com/OpenLiberty/ci.docker/blob/master/docs/icr-images.md[official images^] that
are available from IBM Container Registry (ICR)
