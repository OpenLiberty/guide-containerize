...
You'll learn how to build container images and run containers using https://www.docker.com/[Docker^] for your microservices. You'll construct `Dockerfile` files, create Docker images by using the `docker build` command, and run the image as Docker containers by using `docker run` command.
