...
The [hotspot=add file=0]`ADD` instruction is structured as [hotspot=add file=0]`ADD` [hotspot=userID file=0]`[--chown=<user>:<group>]` [hotspot=source file=0]`<source>` [hotspot=destination file=0]`<destination>`. 
...
In this case, the `inventory` application archive [hotspot=source file=0]`inventory.tar.gz`, created from running `mvn install` previously, is extracted to the destination directory [hotspot=destination file=0]`/opt/ol`. 

The [hotspot=add file=0]`ADD` instruction needs to use the user ID [hotspot=userID file=0]`1001` and group [hotspot=userID file=0]`0` because the `open-liberty` image runs by default with the `USER 1001` (non-root) user for security purposes. Otherwise, the files and directories that are copied over are owned by the root user.
