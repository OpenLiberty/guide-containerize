...
The `system` service can be found in the `start/system` directory, 
and the `inventory` service can be found in the `start/inventory` directory. 
...
Point your browser to the http://localhost:9081/inventory/systems[http://localhost:9080/inventory/systems^] URL. This is the starting point of the `inventory`
...
Next, point your browser to the http://localhost:9081/inventory/systems/localhost[http://localhost:9080/inventory/systems/localhost^] URL.
...
Next,  you will be creating two Docker images for the `inventory` service and `system` service. The first step is to create Dockerfiles for both services.
...
The [hotspot=119-122]`dockerfile-maven` plug-in automatically builds a Docker image from a `Dockerfile` that is located in the same
