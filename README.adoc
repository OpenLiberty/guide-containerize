...
You'll learn how to construct `Dockerfile`, how to create a Docker image by using `docker build`  command, and how to run the image in Docker container 
...
The [hotspot=packageFile]`<packageFile>` tag in the [hotspot]`pom.xml` configures the microservices to be packed in an archive form.
...
The [hotspot=run file=0]`RUN` instruction is structured in two forms as [hotspot=run file=0]`RUN` `<command>` or [hotspot=run file=0]`RUN` `["executable", "parameter1", "parameter2"]`. Whenever the `RUN` instruction is executed, another container layer is created so in order to reduce image size and layer count it is possible to run multiple commands using the `&&` operator. In this case, we are executing the `configure.sh` script which sets up extra configuration for users using Open Liberty images.
...
Next, you will retrieve the `system` containers IP address by using the `system` container name we defined when running the docker containers. 
...
You will find the `[system-ip-address]`:
[role="no_copy"]
----
172.17.0.2
----
...
In this case, the address for `system` is `172.17.0.2`. Take note of the the address `[system-ip-address]` in order to add the system properties to the inventory service. 
