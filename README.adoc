...
You'll learn how to construct `Dockerfile`, how to create Docker images by using `docker build`  command, and how to run the image in Docker container 
by using `docker run` command.

The two  microservices that you'll be working are called `system` and `inventory`. The `system` microservice returns the JVM system properties 
